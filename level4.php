...
require_once('level4challenges.php');


if (isset($_GET['queryTarget']))
{
	if ($_GET['queryTarget'] == 'movies')
	{
		$query = "select * from movies where title = '" . $_GET['query'] . "'";	
	}

	if ($_GET['queryTarget'] == 'users')
	{
		$query = "select username,password from users where username = '" . $_GET['username'] . "' and password = '" . $_GET['password'] . "'";
	}
...
<h1>Level4: Altering prewritten queries</h1>
...
Query: select * from movies where title = '<input type="text" name="query" size="50" />'; 
<input type="hidden" value="movies" name="queryTarget" />
<input type="submit">
</form>
...
<form action="" method="GET" >
Query: select username,password from users where username ='<input type="text" name="username" size="40" />' 
and password = '<input type="text" name="password" size="40" />';
